...
function setGraph1(com,pol,annee,nom){
	...
	request(bbb, function(data){
		...
		request(ddd,function(data, valueCom_){
			...
			var data2 = parseDataHighchartsBar(final,nom);
			graph1.destroy()
			graph1 = createBar(ctx, data, options)
			chart.destroy()
			chart = new Highcharts.Chart(data2);
				},valueCom_)
...
function setGraph2(com,pol,annee,nom){
	...
	request(adx, function(data){
		...
		var data2 = parseDataHighchartsPie(valueCom,nom);
		graph3.destroy()
...
function setGraph3(com,pol,annee,nom){
	...
	request(adx, function(data){
		...
		request(adx2,function(data, valueCom_){
			...
			var data22 = parseDataHighchartsSpider(final,nom);
			console.log(data2);
			graph2.destroy()
			graph2 = createRadar(ctx2, data2, options2)
			chart3.destroy()
			chart3 = new Highcharts.Chart(data22);
		},valueCom_)
	});
}
...
var parseDataHighchartsBar = function(data, nom) {
	// => recuperer l'array du dropdown
	var labels = defautPol;
	var datasets = [];
  var i = 0;
	...
	var datafinal = hcOptions;
	datafinal.title.text = nom;
	datafinal.xAxis.categories = labels;
	datafinal.series = datasets;
	//console.log(datafinal);
	return datafinal
}
var parseDataHighchartsPie = function(data,nom) {
	var labels = ["AGRISY","ENERGIE","RETECI","TROUTE","AUTREST","INDUST", "NON_FR"];
	var options = {
		backgroundColor: [
			"#FF6384",
			"#36A2EB",
...
var parseDataHighchartsSpider = function(data,nom) {
	var labels = ["AGRISY","ENERGIE","RETECI","TROUTE","AUTREST","INDUS"];
	var datasets = [];
	var i = 0;
	for (annee in data){
	...
	var datafinal = hcOptions3;
	datafinal.title.text = nom + ' ' + nomPol;
	datafinal.xAxis.categories = labels;
	datafinal.series = datasets;
	//console.log(datafinal);
	return datafinal
